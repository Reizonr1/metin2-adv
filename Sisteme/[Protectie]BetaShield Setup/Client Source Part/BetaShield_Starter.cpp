...
std::string TRAMPOLINE_CPythonNetworkStream_GetPhase() {
	return CPythonNetworkStream::Instance().GetPhase();
}
bool TRAMPOLINE_CPythonNetworkStream_SendHack(const char* c_szMsg) {
	return CPythonNetworkStream::Instance().__SendHack(c_szMsg);
}
...
void CBetaShield_Starter::ProcessTriggers() {
	...
	DLL_Wrapper_Func(&TRAMPOLINE_CPythonNetworkStream_GetPhase, 4); // sonradan
	DLL_Wrapper_Func(&TRAMPOLINE_CPythonNetworkStream_SendHack, 5);
