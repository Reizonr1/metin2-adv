...
ACMD(do_open_offline_shop)
{
	...
	if (ch->IsDead())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Dead man can not open offline shop");
		return;
	}
	...
	if (ch->GetExchange())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You are exchanging with someone. At first close to the window!");
		return;
	}
	...
	if (ch->GetMyShop())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You had open private shop. At first you must be close this private shop!");
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't open offline shop because you are already look at one offline shop.");
		return;
	}
	...
	if (ch->IsCubeOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't open offline shop because your cube window is open");
		return;
	}
	...
	if (ch->IsOpenSafebox())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't open offline shop because your safebox window is open!");
		return;
	}
	...
	if (ch->GetShop())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't open offline shop because your shop window is open!");
		return;
	}
