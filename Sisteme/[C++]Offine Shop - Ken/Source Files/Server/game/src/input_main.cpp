...
int CInputMain::OfflineShop(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	switch (p->subheader)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Add Item %d", ch->GetOfflineShopStatus());
			// If the user is not update the client, give a message ;)
			if (ch->GetOfflineShopStatus() == 0 || ch->GetOfflineShopStatus() == 1)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "You can't use this window now!");
				return 0;
			}
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Remove Item %d", ch->GetOfflineShopStatus());
			// If the user is not update the client, give a message ;)
			if (ch->GetOfflineShopStatus() == 0 || ch->GetOfflineShopStatus() == 1)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "You can't use this window now!");
		...
		case SHOP_SUBHEADER_CG_REFRESH:
			ch->ChatPacket(CHAT_TYPE_INFO, "Refresh %d", ch->GetOfflineShopStatus());
			if (ch->GetOfflineShopStatus() == 0 || ch->GetOfflineShopStatus() == 1)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "You can't use this window now!");
				return 0;
