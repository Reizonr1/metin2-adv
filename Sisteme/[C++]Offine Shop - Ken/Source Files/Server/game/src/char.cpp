...
#endif
{
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "This item can not sell!");
				return;
			}
	...
	// Insert
	char szQuery2[512];
	snprintf(szQuery2, sizeof(szQuery2), "INSERT INTO %soffline_shop_npc(owner_id, sign, name, time, x, y, z, mapIndex, channel) VALUES(%u, '%s', '%s', %d, %ld, %ld, %ld, %ld, %d)", get_table_postfix(), GetPlayerID(), szSign, GetName(), iTime, GetX(), GetY(), GetZ(), GetMapIndex(), g_bChannel);
	...
	snprintf(szName, sizeof(szName), "%s's offline shop", GetName());
	npc->SetName(szName);
	// End Of Set Name
