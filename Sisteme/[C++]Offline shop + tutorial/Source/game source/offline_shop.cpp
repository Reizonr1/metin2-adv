...
int COfflineShop::Buy(LPCHARACTER ch, BYTE bPos)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Çevrimdýþý çevrimdýþý magazin isimli üye çevrimdýþý.");
	...
	{	
		if (tch)
		{
			...
			if (bIsOverFlow)
				DBManager::instance().DirectQuery("UPDATE player.player SET money = money + %u WHERE id = %u", dwPrice, tch->GetPlayerID());
			else
				tch->PointChange(POINT_GOLD, dwPrice, false);
		}
		else
			DBManager::instance().DirectQuery("UPDATE player.player SET money = money + %u WHERE id = %u", dwPrice, ch->GetOfflineShopOwner()->GetOfflineShopRealOwner());
	}
