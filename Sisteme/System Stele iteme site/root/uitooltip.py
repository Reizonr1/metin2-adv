...
class ItemToolTip(ToolTip):
	...
	def __AppendAttackAccePowerInfo(self, minPower, maxPower):
		...
		if maxPower > minPower:
			self.AppendTextLine(locale.TOOLTIP_ITEM_ATT_POWER % (minPower_final, maxPower_final), self.POSITIVE_COLOR)
		else:
			self.AppendTextLine(locale.TOOLTIP_ITEM_ATT_POWER_ONE_ARG % (minPower_final), self.POSITIVE_COLOR)
	...
	def __AppendMagicAttackAcceInfo(self, minMagicAttackPower, maxMagicAttackPower):
		...
		if minMagicAttackPower_final > 0 or maxMagicAttackPower_final > 0:
			if maxMagicAttackPower_final > minMagicAttackPower_final:
				self.AppendTextLine(locale.TOOLTIP_ITEM_MAGIC_ATT_POWER % (minMagicAttackPower_final, maxMagicAttackPower_final), self.POSITIVE_COLOR)
			else:
				self.AppendTextLine(locale.TOOLTIP_ITEM_MAGIC_ATT_POWER_ONE_ARG % (minMagicAttackPower_final), self.POSITIVE_COLOR)					
		...
		## 코스츔 아이템 ##
			...
			if isCostumeAcce != 0:
				if metinSlot != 0:
					...
					if absChance > 0:
						self.AppendSpace(5)
						self.AppendTextLine(locale.ACCE_ABSORB_CHANCE % (absChance), self.CONDITION_COLOR)
